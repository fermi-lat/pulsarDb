...
namespace pulsarDb {
  ...
  double OrbitalEph::calcOrbitalPhase(const timeSystem::AbsoluteTime & ev_time, double phase_offset) const {
  ...
  timeSystem::ElapsedTime OrbitalEph::calcOrbitalDelay(const timeSystem::AbsoluteTime & ev_time) const {
    // compute elapsed time from epoch of periastron in seconds
    double delta_second = dt(ev_time);
    ...
    double omega = m_par[OM]
      + m_par[OMDOT] * true_anomaly * m_par[PB] / OrbitalEph::s_two_pi;
